...
import pandas as pd
from graph_utils import construct_RGCN_bigraph_from_smiles, etype_features
...
def construct_RGCN_bigraph_from_smiles_with_species_gender_exposure(smiles, duration): ##change
    g = DGLGraph()
    ...
    atoms_feature_all = []
###########################################################################################################    
    #species_feature_all = []    ##change

    duration_feature_all = []     ##change
    
    #species_feature = [0]*8        ##change
    #species_feature[species] = 1     ##change
    


    duration_feature = [duration]   ##change
    
    for atom_index, atom in enumerate(mol.GetAtoms()):
        atom_feature = atom_features(atom).tolist()
        atoms_feature_all.append(atom_feature)
############################################################################################################        
        #species_feature_all.append(species_feature)   ##change
        duration_feature_all.append(duration_feature)  #  
        
    atoms_tensor = torch.tensor(atoms_feature_all)
    
    #species_tensor = torch.tensor(species_feature_all)   #
    duration_tensor = torch.tensor(duration_feature_all)  #

    node_features = torch.cat((atoms_tensor, duration_tensor), dim=1)  #
...
def multi_task_build_dataset_with_species_gender_exposure(dataset_smiles, labels_list, 
                                        smiles_name, duration_list):  ##change
    dataset_gnn = []
    failed_molecule = []
    labels = dataset_smiles[labels_list]
    split_index = dataset_smiles['group']
    ...
    for i, smiles in enumerate(smilesList):
        try:
            #################################################################
            g = construct_RGCN_bigraph_from_smiles_with_species_gender_exposure(smiles, duration_list[i])   ##change
            mask = build_mask(labels.loc[i], mask_value=123456)
            molecule = [smiles, g, labels.loc[i], mask, split_index.loc[i]]
            dataset_gnn.append(molecule)
            print('{}/{} molecule is transformed!'.format(i+1, molecule_number))
...
def built_data_and_save_for_splited_with_species_gender_exposure(
        ...
        task_list_selected=None,
    ######################################################################################
        #species_name='species',   ##change
        duration_name='duration'  ##change
         ):
    '''
        origin_path: str
            origin csv data set path, including molecule name, smiles, task
    ...
    smiles_name = 'smiles'
    data_origin = data_origin.fillna(123456)
    labels_list = [x for x in data_origin.columns if x not in ['smiles', 'group',duration_name]]  ##change
    if task_list_selected is not None:
        labels_list = task_list_selected
    data_set_gnn = multi_task_build_dataset_with_species_gender_exposure(dataset_smiles=data_origin, labels_list=labels_list, 
                                     smiles_name=smiles_name, duration_list=data_origin[duration_name])   ##change
